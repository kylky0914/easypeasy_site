...
// labelKey is a ui-dictionary dot-path (defaults to the shared "WhatsApp Us" label),
// so callers pick a localized label without passing raw strings. `text` overrides the
// prefilled WhatsApp message; otherwise the localized default is used.
export default function WhatsAppButton({ text, labelKey = 'common.whatsappUs', className = '' }) {
  const t = useT()
  const message = text ?? t('common.whatsappMessage')
